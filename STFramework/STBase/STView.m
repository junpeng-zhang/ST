...
-(void)regEvent{
    ...
    if(self.UITextList!=nil)
    {
        ...
        for (id ui in self.UITextList)
        {
            ...
            {
                //UITextView *textView=(UITextView*)ui;
                //textView.tag=-999;//用于ContentSize的高和文本框的差值
                //文本修改事件
...
-(void)moveTextView
{
       ...
       && CGPointEqualToPoint(CGPointZero, self.OriginFrame.origin))
    {
        ...
        if(self.EditingTextUI.absY*Ypt+self.EditingTextUI.frame.size.height+self.KeyboardHeight>STScreeHeightPt)
        {
...
-(NSMutableDictionary *)formData:(id)superView
{
    ...
    for (NSString*key in self.UIList) {
        ...
        {
            [formData setObject:[ui value] forKey:key];
        }
    }
    return formData;
}
